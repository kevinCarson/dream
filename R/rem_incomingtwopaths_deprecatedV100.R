...
#' @title Compute Butts' (2008) Incoming Two Paths Network Statistic for Event Dyads in a Relational Event Sequence
#' @name computeITP
...
#' @import data.table
#' @return The vector of incoming two path statistics for the relational event sequence.
#' @export
...
#'
#' `computeITP()` has been deprecated starting on version 1.0.0 of the `dream` package. Please use the `remstats_triads()` function and see the `NEWS.md` file for more details.
#'
#'
#' The function computes the incoming two path (ITP) network sufficient statistic for
#' a relational event sequence (see Lerner and Lomi 2020; Butts 2008). In essence, the incoming two paths measure captures the tendency of triadic closure to occur in the network of past events, in which the past triadic closure is based upon the incoming two paths structure (see Butts 2008 for an empirical example).
#' This measure allows for ITP scores to be only  computed for the sampled
...
#'
#'
#'@details The function calculates incoming two paths scores for relational
...
computeITP <- function(observed_time, # variable (column) name that contains the time variable
  ...
  lifecycle::deprecate_warn("1.0.0", "computeITP()", "remstats_triads()")
  ...
  if (sliding_windows == FALSE) {
    ...
    for (i in 1:n_observed_events) { # for all observed events in the dataset
      ...
      if(nrow(jB) == 0){next}#no past events
      jB$i_endpoint <- jB$receiver# <- jB[, i_endpoint := receiver] # making a new variable, i_endpoint, that is the non-current-reciever actor

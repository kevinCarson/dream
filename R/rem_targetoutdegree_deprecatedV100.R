...
#' @title Compute the Outdegree Network Statistic for Event Receivers in a Relational Event Sequence
#' @name computeReceiverOutdegree
...
#' @import data.table
#' @return The vector of receiver outdegree statistics for the relational event sequence.
#' @export
...
#'
#' `computeReceiverOutdegree()` has been deprecated starting on version 1.0.0 of the `dream` package. Please use the `remstats_degree()` function and see the `NEWS.md` file for more details.
#'
#'
#'
#' The function computes the receiver outdegree network sufficient statistic for
...
#'
#'
#'@details The function calculates reciever outdegree scores for relational event
...
#'
#'That is, all past events in which the past receiver is the current sender and
#'the event receiver can be any past actor.
#'
...
computeReceiverOutdegree <- function(observed_time, # variable (column) name that contains the time variable
  ...
  #
  ########################################################
  lifecycle::deprecate_warn("1.0.0", " computeReceiverOutdegree()", "remstats_degree()")
  ...
  if (sliding_windows == FALSE) {
    ...
    for (i in 1:n_observed_events) { # for all observed events in the dataset
      ...
      ####### For repetition!
      outdegreei <- events[events$sender == receiveri] # all events that have the current article and user
      if(nrow( outdegreei) == 0){next}
      outdegreei <- outdegreei[outdegreei$time < timei]
      if(nrow( outdegreei) == 0){next}
      outdegreei <- remExpWeights(current = timei, past = outdegreei$time,
  ...
  if (sliding_windows == TRUE) {
    ...
    for (j in 1:n_blocks) { # go through each of the blocks (i.e., subsetting the datamatrix to only handle
      ...
      for (i in new_start:n_sampled) { # for i in all sampled events
        ...
        timei <- eventSet$time[i] ####  current time for event i
        ####### For repetition!
        outdegreei <- eventsj[eventsj$sender == receiveri] # all events that have the current article and user
        if(nrow( outdegreei) == 0){next}
        ...
        if(nrow( outdegreei) == 0){next}
          ...
          if(counts == TRUE){ #if the user wants the counts of events to be returned
            outdegree[i] <- length(outdegreei) + outdegree[i] # add the sum of the past event weights to the ith spot in the inertia vector
          }else{
            outdegree[i] <- sum(outdegreei) + outdegree[i] # add the sum of the past event weights to the ith spot in the inertia vector
          }
        ###### clearing memory for consumption purposes
        rm(list = c("receiveri", "timei", "outdegreei"))
      }
      rm(list = c("eventsj"))
    }
  }

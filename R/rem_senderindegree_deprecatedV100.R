...
#' @title Compute the Indegree Network Statistic for Event Senders in a Relational Event Sequence
#' @name computeSenderIndegree
...
#' @import data.table
#' @return The vector of sender indegree statistics for the relational event sequence.
#' @export
...
#'
#' `computeSenderIndegree()` has been deprecated starting on version 1.0.0 of the `dream` package. Please use the `remstats_degree()` function and see the `NEWS.md` file for more details.
#'
#'
...
#'
#'Following Butts (2008), if the counts of the past events are requested, the formula for sender indegree for
#'event \eqn{e_i} is:
#'\deqn{sender indegree_{e_{i}} = d(r' = s, t') }
#'Where, \eqn{d()} is the number of past events where the event receiver, *r'*, is the current event sender *s* . Moreover, the counting equation
...
computeSenderIndegree <- function(observed_time, # variable (column) name that contains the time variable
  ...
  #
  ########################################################
  lifecycle::deprecate_warn("1.0.0", " computeSenderIndegree()", "remstats_degree()")
  ...
  if (sliding_windows == FALSE) {
    ...
    indegree <- numeric(n_observed_events) # empty vector to store computed statistics
    for (i in 1:n_observed_events) { # for all observed events in the dataset
      ...
      ####### For repetition!
      indegreei <- events[events$receiver == senderi] # all events that have the current article and user
      ...
      if(nrow(indegreei) == 0){next}
      indegreei <- remExpWeights(current = timei,
                                      past = indegreei$time,
                                      halflife = halflife,
                                      dyadic_weight = dyadic_weight,
                                      Lerneretal_2013 = Lerneretal_2013,
                                      exp.weights = TRUE)
      #indegreei[, .(redr:::weight(current = timei, past = time, halflife = halflife, dyadic_weight = dyadic_weight, Lerneretal_2013 = Lerneretal_2013))]
      if(counts == TRUE){ #if the user wants the counts of events to be returned
        indegree[i] <- length(indegreei)  # add the sum of the past event weights to the ith spot in the inertia vector
        }else{
        indegree[i] <- sum(indegreei)  # add the sum of the past event weights to the ith spot in the inertia vector
      }
      rm(list = c("indegreei"))
    }
  }
  ...
  if (sliding_windows == TRUE) {
    ...
    for (j in 1:n_blocks) { # go through each of the blocks (i.e., subsetting the datamatrix to only handle
      ...
      for (i in new_start:n_sampled) { # for i in all sampled events
        ...
        timei <- eventSet$time[i] ####  current time for event i
        ####### For repetition!
        indegreei <- eventsj[eventsj$receiver == senderi] # all events that have the current article and user
        if(nrow(indegreei) == 0){next}
        ### for all events that occurred before the current, compute the weight with a dyadic cutoff at dyadic_weighti
        indegreei <- indegreei[indegreei$time < timei]
        if(nrow(indegreei) == 0){next}
        ...
        if(counts == TRUE){ #if the user wants the counts of events to be returned
          indegree[i] <- length(indegreei) + indegree[i]  # add the sum of the past event weights to the ith spot in the inertia vector
        }else{
          indegree[i] <- sum(indegreei) + indegree[i] # add the sum of the past event weights to the ith spot in the inertia vector
        }
         ###### clearing memory for consumption purposes
        rm(list = c("senderi", "timei", "indegreei"))
      }
      rm(list = c("eventsj"))
    }
  }

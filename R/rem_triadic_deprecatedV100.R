...
#' @title Compute the Triadic Closure Network Statistic for Event Dyads in a Relational Event Sequence
#' @name computeTriads
...
#' @import data.table
#' @return The vector of triadic closure network statistics for the relational event sequence.
#' @export
...
#'
#' `computeTriads()` has been deprecated starting on version 1.0.0 of the `dream` package. Please use the `remstats_triads()` function and see the `NEWS.md` file for more details.
#'
#'
#' This function computes the triadic closure network sufficient statistic for
#' a relational event sequence (see Lerner and Lomi 2020; Butts 2008).
#' This measure allows for triadic scores to be only  computed for the sampled
...
#'
#'
#'@details This function calculates triadic scores for relational event
...
#'
#'The general formula for triadic structures for event \eqn{e_i} is:
#'\deqn{triadic_{e_{i}} = \sqrt{ \sum_k w(s, r', t) \cdot w(s', r, t) }}
#'
#'That is, this function combines all triadic structures discussed in Butts (2008)
#'into a single summation such that the computed scores include incoming shared
#'partners, outgoing shared partners, incoming two paths, and outgoing two paths.
#'
...
#'
#'
#'Following Butts (2008), if the counts of the past events are requested, the formula for triadic structures for
#'event \eqn{e_i} is:
#'\deqn{TS_{e_{i}} = \sum_{i=1}^{|H|} \min\left[d(s,r',t), d(s',r,t)\right]}
#'where, \eqn{d()} is the number of past events that meet the specific set operations. Notably, this function combines all triadic
#'structures discussed in Butts (2008) into a single summation, such that the computed scores include incoming shared
#'partners, outgoing shared partners, incoming two paths, and outgoing two paths. The sum loops through all
#'unique actors that have formed past sent or received ties from the current event sender and receiver.
...
computeTriads <- function(observed_time, # variable (column) name that contains the time variable
  ...
  #
  ########################################################
  lifecycle::deprecate_warn("1.0.0", "computeTriads()", "remstats_triads()")
  ...
  if (sliding_windows == FALSE) {
    ...
    for (i in 1:n_observed_events) { # for all observed events in the dataset
      ...
      Ai <- events[events$sender == senderi | events$receiver == senderi] # past events that the current sender is involved in
      if(nrow(Ai) == 0){next}

      Ai <- Ai[Ai$time < timei] # all events prior to the current event
      if(nrow(Ai) == 0){next}

      Ai <- Ai[Ai$receiver != receiveri | Ai$sender != receiveri] # removing events in which the current receiver is involved
      if(nrow(Ai) == 0){next}
      #Ai <- Ai[, i_endpoint := data.table::fifelse(sender == senderi, receiver, sender)] # making a new variable, i_endpoint, that is the non-current-sender actor
      Ai$i_endpoint <- data.table::fifelse(Ai$sender == senderi, Ai$receiver, Ai$sender)

      jB <- events[events$sender == receiveri | events$receiver == receiveri] # past events that the current receiver is involved in
      if(nrow(jB) == 0){next}

      jB <- jB[jB$time < timei] # all events prior to the current event
      if(nrow(jB) == 0){next}

      jB <- jB[jB$sender != senderi | jB$receiver != senderi] # removing events in which the current sender is involved
      if(nrow(jB) == 0){next}
      #jB <- jB[, i_endpoint := data.table::fifelse(sender == receiveri, receiver, sender)] # making a new variable, i_endpoint, that is the non-current-reciever actor
      jB$i_endpoint <- data.table::fifelse(jB$sender == receiveri, jB$receiver, jB$sender)
      ...
      Ai$wai <- remExpWeights(current = timei, past = Ai$time,
                              halflife = halflife, dyadic_weight = dyadic_weight,
      ...
      jB$wib <- remExpWeights(current = timei, past = jB$time,
                              halflife = halflife, dyadic_weight = dyadic_weight,
                              Lerneretal_2013 = Lerneretal_2013,
                              exp.weights = TRUE)

      if(sum(jB$wib) == 0){next} #All weights would zero out so skip
      jB <- jB[jB$wib != 0] #getting rid of the zero weight actors
      if(counts == TRUE){
        counti <- 0
  ...
  if (sliding_windows == TRUE) {
    ...
    for (j in 1:n_blocks) { # for all blocks from the sliding windows framework
      ...
      for (i in new_start:new_stop) { # for all sampled events in block j
        ...
        ####### For reciprocity!
        #### Building all of the open triangles! (Ai, iA, Bj, jB)
        Ai <- eventsj[eventsj$sender == senderi | eventsj$receiver == senderi] # past events that the current sender is involved in
        if(nrow(Ai) == 0){next}
        ...
        jB <- eventsj[eventsj$sender == receiveri | eventsj$receiver == receiveri] # past events that the current receiver is involved in
        if(nrow(jB) == 0){next}

...
#' @title Compute Butts' (2008) Outgoing Two Paths Network Statistic for Event Dyads in a Relational Event Sequence
#' @name computeOTP
...
#' @import data.table
#' @return The vector of outgoing two path statistics for the relational event sequence.
#' @export
#'
#' @description
#' `r lifecycle::badge("deprecated")`
#'
#' `computeOTP()` has been deprecated starting on version 1.0.0 of the `dream` package. Please use the `remstats_triads()` function and see the `NEWS.md` file for more details.
#'
#'
#' The function computes the outgoing two paths (OTP) network sufficient statistic for
#' a relational event sequence (see Lerner and Lomi 2020; Butts 2008). In essence, the outgoing two paths measure captures the tendency of triadic closure to occur in the network of past events, in which the past triadic closure is based upon the outgoing two paths structure (see Butts 2008 for an empirical example).
#' This measure allows for OTP scores to be only  computed for the sampled
...
#'
#'@details The function calculates the outgoing two paths statistic for relational
...
computeOTP <- function(observed_time, # variable (column) name that contains the time variable
  ...
  #
  ########################################################
  lifecycle::deprecate_warn("1.0.0", "computeOTP()", "remstats_triads()")
  ...
  if (sliding_windows == FALSE) {
    ...
    for (i in 1:n_observed_events) { # for all observed events in the dataset
      ...
      if(nrow(Ai) == 0){next}#no past events
      Ai$i_endpoint <- Ai$receiver #Ai[, i_endpoint := receiver] # making a new variable, i_endpoint, that is the non-current-sender actor
  ...
  if (sliding_windows == TRUE) {
    ...
    sliding_window$minimum_times <- remExpWeights(sliding_window$time, dyadic_weight = dyadic_weight,

...
#'
#' `computeReciprocity()` has been deprecated starting on version 1.0.0 of the `dream` package. Please use the `remstats_reciprocity()` function and see the `NEWS.md` file for more details.
#'
#'
...
computeReciprocity <- function(observed_time, # variable (column) name that contains the time variable
  ...
  lifecycle::deprecate_warn("1.0.0", "computeReciprocity()", "remstats_reciprocity()")
  ########################################################
  #
  ...
  if (sliding_windows == FALSE) {
    #base::cat("Starting computation of reciprocity scores without the sliding windows framework.......") # outputting status to user
    n_observed_events <- nrow(eventSet) # number of observed events in dataset
    reciprocity <- numeric(n_observed_events) # empty vector to store computed statistics
    for (i in 1:n_observed_events) { # for all observed events in the dataset
      ...
      ### B -> A
      senderi <- eventSet$receiver[i] ####  current user for event i
      receiveri <- eventSet$sender[i] ####  current article for event i
      timei <- eventSet$time[i] ####  current time for event i
      ####### For repetition!
      reciprocityi <- events[events$sender == senderi & events$receiver == receiveri] # all events that have the current article and user
      if(base::nrow(reciprocityi) == 0){next}
      ### for all events that occurred before the current, compute the weight with a dyadic cutoff at dyadic_weighti
      reciprocityi <- reciprocityi[reciprocityi$time < timei]
      if(base::nrow(reciprocityi) == 0){next}
      reciprocityi <- remExpWeights(current = timei,
                                    past = reciprocityi$time,
                                    halflife = halflife, dyadic_weight = dyadic_weight,
                                    Lerneretal_2013 = Lerneretal_2013)

        #reciprocityi[, .(remExpWeights(current = timei, past = time, halflife = halflife, dyadic_weight = dyadic_weight, Lerneretal_2013 = Lerneretal_2013))]


      if(counts == TRUE){ #if the user wants the counts of events to be returned
        reciprocity[i] <- length(reciprocityi)
      }else{#if the user wants the exponential weighting function to be applied
      reciprocity[i] <- sum(reciprocityi) # add the sum of the past event weights to the ith spot in the inertia vector
      }
      ###### clearing memory for consumption purposes
      rm(list = c("senderi", "receiveri", "timei", "reciprocityi"))
    }
  }
  ...
  if (sliding_windows == TRUE) {
    ...
    reciprocity <- rep(0, n_observed_events) # empty vector to store computed statistics
    eventIDS <- processed_seqIDs # the sampled eventnet IDS

    ######################################################################
    ####### Computing Statistics on a Sliding Frame Method
    ######################################################################
    #base::cat("Starting computation of reciprocity scores with sliding windows framework.......") # outputting status to user
    ...
    for (j in 1:n_blocks) { # go through each of the blocks (i.e., subsetting the datamatrix to only handle
      ...
      for (i in new_start:n_sampled) { # for i in all sampled events
        ...
        timei <- eventSet$time[i] ####  current time for event i
        ####### For repetition!
        reciprocityi <- eventsj[eventsj$sender == senderi & eventsj$receiver == receiveri] # all events that have the current article and user
        if(base::nrow(reciprocityi) == 0){next}
        ...
        #reciprocityi[, .(remExpWeights(current = timei, past = time, halflife = halflife, dyadic_weight = dyadic_weight, Lerneretal_2013 = Lerneretal_2013))]
          ...
          if(counts == TRUE){ #if the user wants the counts of events to be returned
            reciprocity[i] <- length(reciprocityi) + reciprocity[i] # add the sum of the past event weights to the ith spot in the inertia vector
          }else{
          reciprocity[i] <- sum(reciprocityi) + reciprocity[i] # add the sum of the past event weights to the ith spot in the inertia vector
          }
        ###### clearing memory for consumption purposes
        rm(list = c("senderi", "receiveri", "timei", "reciprocityi"))
      }
  ...
  #
  ########################################################
  return(reciprocity)# return the vector of values
}

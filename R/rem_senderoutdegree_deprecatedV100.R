...
#' @title Compute the Outdegree Network Statistic for Event Senders in a Relational Event Sequence
#' @name computeSenderOutdegree
...
#' @import data.table
#' @return The vector of sender outdegree statistics for the relational event sequence.
#' @export
#'
#'
...
#'
#' `computeSenderOutdegree()` has been deprecated starting on version 1.0.0 of the `dream` package. Please use the `remstats_degree()` function and see the `NEWS.md` file for more details.
#'
#'
#'
#' The function computes the sender outdegree network sufficient statistic for
#' a relational event sequence (see Lerner and Lomi 2020; Butts 2008).
#' This measure allows for outdegree scores to be only  computed for the sampled
...
#'
#'
#'@details The function calculates sender outdegree scores for relational event
...
#'the event target can be any past user.
#'
...
#'
#'Following Butts (2008), if the counts of the past events are requested, the formula for sender outdegree for
#'event \eqn{e_i} is:
#'\deqn{sender outdegree_{e_{i}} = d(s = s', t') }
#'Where, \eqn{d()} is the number of past events where the sender *s'* is the current event sender, *s*. Moreover, the counting equation
#'can be used in tandem with relational relevancy, by specifying the halflife parameter, exponential
#'weighting function, and the dyadic cutoff weight values. If the user is not interested in modeling
...
computeSenderOutdegree <- function(observed_time, # variable (column) name that contains the time variable
  ...
  #
  ########################################################
  lifecycle::deprecate_warn("1.0.0", "computeSenderOutdegree()", "remstats_degree()")
  ...
  ########################################################
  rm(list = c("observed_time", "observed_sender", "processed_time", "processed_sender"))
  ...
  if (sliding_windows == FALSE) {
    #base::cat("Starting computation of outdegree scores without the sliding windows framework.......") # outputting status to user
    n_observed_events <- nrow(eventSet) # number of observed events in dataset
    outdegree <- rep(0, n_observed_events) # empty vector to store computed statistics
    for (i in 1:n_observed_events) { # for all observed events in the dataset
      senderi <- eventSet$sender[i] ####  current article for event i
      timei <- eventSet$time[i] ####  current time for event i
      ####### For repetition!
      outdegreei <- events[events$sender == senderi] # all events that have the current article and user
      if(nrow(outdegreei) == 0){next}
      ### for all events that occurred before the current, compute the weight with a dyadic cutoff at dyadic_weighti
      outdegreei <- outdegreei[outdegreei$time < timei]
      if(nrow(outdegreei) == 0){next}
        outdegreei$weight <- remExpWeights(
          current = timei,
          past = outdegreei$time, halflife = halflife,
        ...
        if(counts == TRUE){ #if the user wants the counts of events to be returned
          outdegree[i] <- length(outdegreei$weight) + outdegree[i] # add the sum of the past event weights to the ith spot in the inertia vector
        }else{#if the user wants the exponential weighting function to be applied
          outdegree[i] <- sum(outdegreei$weight) + outdegree[i] # add the sum of the past event weights to the ith spot in the inertia vector
        }
      rm(list = c("outdegreei"))
    }
  }
  ########################################################
  #
  ...
  if (sliding_windows == TRUE) {
    ...
    ######################################################################
    #base::cat("Starting computation of outdegree scores with sliding windows framework.......") # outputting status to user
    ...
    for (j in 1:n_blocks) { # go through each of the blocks (i.e., subsetting the datamatrix to only handle
      ...
      ifelse(j == 1, new_start <- 1,
        new_start <- base::suppressWarnings({ base::min(which(eventIDS >= sliding_window$start_block[j]))})
      )
      ...
      for (i in new_start:n_sampled) { # for i in all sampled events
        ...
        timei <- eventSet$time[i] ####  current time for event i
        ####### For repetition!
        outdegreei <- eventsj[eventsj$sender == senderi] # all events that have the current article and user
        if(nrow(outdegreei) == 0){next}
        ...
        rm(list = c("senderi", "timei", "outdegreei"))
      }
      rm(list = c("eventsj"))
    }
  }
  ...
  #
  ########################################################
  return(outdegree)# return the vector of values

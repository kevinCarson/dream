...
#'
#' `computeRepetition()` has been deprecated starting on version 1.0.0 of the `dream` package. Please use the `remstats_repetition()` function and see the `NEWS.md` file for more details.
#'
...
#'
#'
computeRepetition <-    function(observed_time, # variable (column) name that contains the time variable
  ...
  lifecycle::deprecate_warn("1.0.0", "computeRepetition()", "remstats_repetition()")
  ########################################################
  #
  ...
  if (sliding_windows == FALSE) {
    ...
    for (i in 1:n_observed_events) { # for all observed events in the dataset
      ...
      ####### For repetition!
      repetitioni <- events[ events$sender == senderi & events$receiver == receiveri]
      if(base::nrow(repetitioni) == 0){next}
      repetitioni <- repetitioni[repetitioni$time < timei]
      if(base::nrow(repetitioni) == 0){next}
      ...
      repetitioni$weight <- remExpWeights(current = timei, past = repetitioni$time,
                                               halflife = halflife, dyadic_weight = dyadic_weight,
                                               Lerneretal_2013 = Lerneretal_2013)
      if(counts == TRUE){
        repetition[i] <- length(repetitioni$weight)  # add the sum of the past event weights to the ith spot in the inertia vector
      }else{
  ...
  if (sliding_windows == TRUE) {
    ...
    for (j in 1:n_blocks) { # go through each of the blocks (i.e., subsetting the datamatrix to only handle
      ...
      for (i in new_start:n_sampled) { # for i in all sampled events
        ...
        timei <- eventSet$time[i] ####  current time for event i
        ####### For repetition!
        repetitioni <- eventsj[eventsj$sender == senderi & eventsj$receiver == receiveri] # all events that have the current article and user
        if(base::nrow(repetitioni) == 0){next}
        repetitioni <- repetitioni[repetitioni$time < timei] ### for all events that occurred before the current, compute the weight with a dyadic cutoff at dyadic_weighti
        if(base::nrow(repetitioni) == 0){next}
        repetitioni$weight <- remExpWeights(current = timei, past = repetitioni$time,
                                            halflife = halflife, dyadic_weight = dyadic_weight,
                                            Lerneretal_2013 = Lerneretal_2013)
        if(counts == TRUE){
          repetition[i] <- length(repetitioni$weight) + repetition[i] # add the sum of the past event weights to the ith spot in the inertia vector
        }else{

## The Creation of  Dynamic One-Mode Risk Sets
...
#' @title Process and Create Risk Sets for a One-Mode Relational Event Sequence
#' @name processOMEventSeq
...
processOMEventSeq <- function(data, # full event data sequence
  ...
  lifecycle::deprecate_warn("1.0.0", " processOMEventSeq()", "create_riskset()")
  ########################################################
  #
  ...
  past_actors <- collapse::funique(sender[1:(observed[i] - 1)]) # starting a vector to store all the unique senders in the past event sequence
  past_actorz <- collapse::funique(receiver[1:(observed[i] - 1)]) # starting a vector to store all the  unique receivers in the past event sequence
  past_actors[(length(past_actors) + 1):(length(past_actors) + length(past_actorz))] <- past_actorz
  past_actors <- collapse::funique(past_actors)
  ...
  if (observed[i] == 1) { # Following prior REM software, if the first selected event is the first event, then
    ...
    nullEventsi <- data.table::data.table( # combine this frame with the prior sampled ones
      sender = dqrng::dqsample(past_actors, m, replace = T), # sample from the past senders (i.e., only senders that have appears in the past)
      receiver = dqrng::dqsample(past_actors, m, replace = T), # sample from the past receivers (i.e., only receivers that have appears in the past)
    ...
    # that null event set
    if ((length(past_actors) >= n_controls)) {
      enough_actors <- TRUE
    }
    ...
    if (enough_actors == FALSE) {
      ...
      possible_combinations <- base::expand.grid(past_actors, past_actors)
    ...
    if (enough_actors == FALSE) { # Now, if after those two checks, we still do not have enough past actors
      ...
      possible_combinations <- data.table::data.table(base::expand.grid(
        sender = past_actors,
        receiver = past_actors
      ))

      whichABequal <- collapse::whichv(possible_combinations$sender, possible_combinations$receiver)
      possible_combinations <- possible_combinations[-whichABequal]
    ...
    if (nDuplicated != 0 & enough_actors == TRUE) { # Now, if we do have enough actors, AND we have duplicated controls (that is,
      ...
      while (nDuplicated != 0) { # while we still have duplicated cases
        ...
        whichABequal <- collapse::whichv(nullEventsi$sender, nullEventsi$receiver) # which senders = receivers (Self loops cannot happen in event models)
        whichduplicated <- collapse::funique(c(whichABequal, whichduplicated))
        ...
        redoNull <- data.table::data.table( # combine this frame with the prior sampled ones
          sender = dqrng::dqsample(past_actors, nDuplicated, replace = T), # sample from the past senders (i.e., only senders that have appears in the past)
          receiver = dqrng::dqsample(past_actors, nDuplicated, replace = T), # sample from the past receivers (i.e., only receivers that have appears in the past)
        ...
        whichABequal <- collapse::whichv(nullEventsi$sender, nullEventsi$receiver)
        nDuplicated <- nDuplicated + length(whichABequal)
      }
    }
    ...
    if (nDuplicated != 0) { # if we do a duplicated case: that is, a control is the same event as the observed event
      ...
      if (enough_actors == TRUE) { # if we do have enough cases to populate the full control risk set
        ...
        while (duplicatedValue == 1) { # while we still have duplicated events
          ...
          fulleventsi$sender[duplicatedID] <- dqrng::dqsample(past_actors, nDuplicated, replace = T)
          fulleventsi$receiver[duplicatedID] <- dqrng::dqsample(past_actors, nDuplicated, replace = T)
          Duplicated <- base::duplicated(fulleventsi, by = c("sender", "receiver")) # are any of these values duplicates of the observed event?
          nDuplicated <- sum(Duplicated) # the number of duplicated events (this should be equal to 1 if there is a duplicate; 0 else)
          if (nDuplicated == 0) {
  ...
  for (i in 2:length(observed)) { # for all events from 2 to the last event
    ...
    past_sendersi[(length(past_sendersi) + 1):(length(past_sendersi) + length(past_recieveri))] <- past_recieveri
    past_sendersi <- collapse::funique(past_sendersi)
    different <- collapse::`%==%`(fastmatch::`%fin%`(past_sendersi, past_actors),FALSE) # getting all the senders that have not previously occurred.  [MADE THIS CHECK]
    if (length(different) != 0) {
      past_actors[(length(past_actors) + 1):(length(past_actors) + length(past_sendersi[different]))] <- past_sendersi[different]
    }
    ...
    ################################################################################################
    #### NEED TO EDIT THE ABOVE TO CHECK TO MAKE SURE WE ARENT OVER DOING
    ################################################################################################
    ...
    if (enough_actors == FALSE) { # if
      if ((length(past_actors)) > (n_controls - 1)) { # f we have enough actors
        enough_actors <- TRUE # change value to TRUE
      }

      #### Provide the full Cartesian plot of past senders x past receivers
      possible_combinations <- base::expand.grid(past_actors, past_actors)
      possible_combinations <- possible_combinations[-which(possible_combinations$Var1 == possible_combinations$Var2),]
      if (nrow(possible_combinations) >= n_controls) { # if the number of combinations >= n_controls
        enough_actors <- TRUE # we have enough actors
      }
    ...
    if (nDuplicated != 0 & enough_actors == TRUE) { # Now, if we do have enough actors, AND we have duplicated controls (that is,
      ...
      while (nDuplicated != 0) { # while we still have duplicated cases
        ...
        whichABequal <- collapse::whichv(nullEventsi$sender, nullEventsi$receiver) # which senders = receivers (Self loops cannot happen in event models)
        whichduplicated <- collapse::funique(c(whichABequal, whichduplicated))
        nDuplicated <- length(whichduplicated)
        ##### Create a new data set, called redoNull, that contains the controls to replace the duplicated ones!
        redoNull <- data.table::data.table( # combine this frame with the prior sampled ones
          sender = dqrng::dqsample(past_actors, nDuplicated, replace = T), # sample from the past senders (i.e., only senders that have appears in the past)
          receiver = dqrng::dqsample(past_actors, nDuplicated, replace = T), # sample from the past receivers (i.e., only receivers that have appears in the past)
  ...
  if(time_dependent == TRUE){ #should a time dynamic risk set be created
    ...
    if (observed[i] == 1) { # Following prior REM software, if the first selected event is the first event, then
      ...
      past_actors <- collapse::funique(sender[timesGood]) # starting a vector to store all the unique senders in the past event sequence
      past_actorz <- collapse::funique(receiver[timesGood]) # starting a vector to store all the  unique receivers in the past event sequence
      past_actors[(length(past_actors) + 1):(length(past_actors) + length(past_actorz))] <- past_actorz
      past_actors <- collapse::funique(past_actors)
    ...
    for (i in 2:length(observed)) { # for all events from 2 to the last event
      ...
      past_senders[(length(past_senders) + 1):(length(past_senders) + length(past_reciever))] <- past_reciever
      past_actors <- collapse::funique(past_senders)
      ...
      # and 3 past receivers, then we have at 12 different combinations.
      if (enough_actors == FALSE) { # if
        if ((length(past_actors)*(length(past_actors)-1)) > (n_controls*(n_controls-1)-1)) { # f we have enough actors
          enough_actors <- TRUE # change value to TRUE
        }

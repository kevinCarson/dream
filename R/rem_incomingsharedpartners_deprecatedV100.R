...
#' @title Compute Butts' (2008) Incoming Shared Partners Network Statistic for Event Dyads in a Relational Event Sequence
#' @name computeISP
...
#' be used to compute the resulting network statistic, while FALSE indicates the ap- proach will not be used. Set
...
#' @import data.table
#' @return The vector of incoming shared partner statistics for the relational event sequence.
#' @export
#'
#'
#' @description
...
#'
#' `computeISP()` has been deprecated starting on version 1.0.0 of the `dream` package. Please use the `remstats_triads()` function and see the `NEWS.md` file for more details.
#'
#'
#' This function calculates the incoming shared partners (ISP) network sufficient statistic for
#' a relational event sequence (see Lerner and Lomi 2020; Butts 2008). In essence, the incoming shared partners measure captures the tendency of triadic closure to occur in the network of past events, in which the past triadic closure is based upon the incoming shared partners structure (see Butts 2008 for an empirical example).
#' This measure allows for ISP scores to be computed only for the sampled
...
#'
#'
#'@details This function calculates incoming shared partners scores for relational
...
#'Moreover, the counting equation can leverage relational relevancy by specifying the halflife parameter, exponential
...
computeISP <- function(observed_time, # variable (column) name that contains the time variable
  ...
  #
  ########################################################
  lifecycle::deprecate_warn("1.0.0", "computeISP()", "remstats_triads()")
  ...
  if (sliding_windows == FALSE) {
    ...
    for (i in 1:n_observed_events) { # for all observed events in the dataset
      ...
      Ai <- events[events$receiver == senderi] # past events that the current sender is involved in
      if(nrow(Ai) == 0){next}#no past events
      Ai <- Ai[Ai$time < timei] # all events prior to the current event
      if(nrow(Ai) == 0){next}#no past events
      Ai <- Ai[Ai$sender != receiveri] # removing events in which the current receiver is involved
      if(nrow(Ai) == 0){next}#no past events
      Ai$i_endpoint <-  Ai$sender #<- Ai[, i_endpoint := sender]  [original DT code] # making a new variable, i_endpoint, that is the non-current-sender actor
      jB <- events[events$receiver == receiveri] # past events that the current receiver is involved in
      if(nrow(jB) == 0){next}#no past events
      jB <- jB[jB$time < timei] # all events prior to the current event
      if(nrow(jB) == 0){next}#no past events
      jB <- jB[jB$sender != senderi] # removing events in which the current receiver is involved
      if(nrow(jB) == 0){next}#no past events
      jB$i_endpoint <- jB$sender # <- jB[, i_endpoint := sender] # making a new variable, i_endpoint, that is the non-current-reciever actor
  ...
  if (sliding_windows == TRUE) {
    ...
    for (j in 1:n_blocks) { # for all blocks from the sliding windows framework
      ...
      for (i in new_start:new_stop) { # for all sampled events in block j
        ...
        ####### For reciprocity!
        #### Building all of the open triangles! (Ai, iA, Bj, jB)
        Ai <- eventsj[eventsj$receiver == senderi] # past events that the current sender is involved in
        if(nrow(Ai) == 0){next}#no past events
        Ai <- Ai[Ai$time < timei] # all events prior to the current event
        if(nrow(Ai) == 0){next}#no past events
        Ai <- Ai[Ai$sender != receiveri] # removing events in which the current receiver is involved
        if(nrow(Ai) == 0){next}#no past events
        Ai$i_endpoint <- Ai$sender #Ai[, i_endpoint := sender] # making a new variable, i_endpoint, that is the non-current-sender actor
        jB <- eventsj[eventsj$receiver == receiveri] # past events that the current receiver is involved in
        if(nrow(jB) == 0){next}#no past events
        jB <- jB[jB$time < timei] # all events prior to the current event
        if(nrow(jB) == 0){next}#no past events
        jB <- jB[jB$sender != senderi] # removing events in which the current receiver is involved

...
// [[Rcpp::export]]
NumericVector computeoutsharedpart(NumericVector time,
  ...
  for (int i = 0; i < nevents; i++) {
    ...
    if(sampledevent[i] == 1){ // if the event is sampled, we need to compute the statistics
      ...
      std::vector<std::string>& target_alters = sender_neighbors[curTarget];
      ...
      if(nshared > 0){ //if there are shared actors then we need to update the weights
        ...
        for (int z = 0; z < nshared; z++) { //for each shared actor
          ...
          std::string dyadA = curSender + appender +curactor; //combining the strings (s,h)
          std::string dyadB = curTarget + appender +curactor; //combining the strings (h,r)

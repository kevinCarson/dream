...
// [[Rcpp::export]]
NumericVector computetriadsrem(NumericVector time,
  ...
  std::vector<std::string> sender_alters; 
  std::vector<std::string> target_alters; 
  std::vector<std::string> shared_actors; //an empty vector to stores the shared nodes
  std::string dyadA; 
  std::string dyadB;
  std::string curactor;
  std::string curSender;
  ...
  for (int i = 0; i < nevents; i++) {
    ...
    if(sampledevent[i] == 1){ // if the event is sampled, we need to compute the statistics
      ...
      curSender = Rcpp::as<std::string>(sender[i]); // the current event sender
      curTarget = Rcpp::as<std::string>(target[i]); // the current event target
      if(tri_type == "outgoing.two.paths"){
          // this extracts all of the past actors that the current sender sent a tie to
          sender_alters = sender_neighbors[curSender];
          target_alters = target_neighbors[curTarget];
      }
      if(tri_type == "incoming.shared.partners"){
        // this extracts all of the past actors that the current sender sent a tie to
          sender_alters = target_neighbors[curSender];
          target_alters = target_neighbors[curTarget];
      }
      if(tri_type == "incoming.two.paths"){
        // this extracts all of the past actors that the current sender sent a tie to
        sender_alters = target_neighbors[curSender];
        target_alters = sender_neighbors[curTarget];
      }
      if(tri_type == "outgoing.shared.partners"){
        // this extracts all of the past actors that the current sender sent a tie to
        sender_alters = sender_neighbors[curSender];
        target_alters = sender_neighbors[curTarget];
      }
      shared_actors.clear(); // clearing the vector for this specific i dyad
      ...
      if(nshared > 0){ //if there are shared actors then we need to update the weights
        ...
        for (int z = 0; z < nshared; z++) { //for each shared actor
         ...
         curactor = shared_actors[z]; //the current shared actor
          if(tri_type == "outgoing.two.paths"){
            // this extracts all of the past actors that the current sender sent a tie to
             dyadA = curSender + appender +curactor; //combining the strings (s,h)
             dyadB = curactor + appender +curTarget; //combining the strings (h,r)
          }
          if(tri_type == "incoming.shared.partners"){
            // this extracts all of the past actors that the current sender sent a tie to
            dyadA = curactor + appender +curTarget; //combining the strings (s,h)
            dyadB = curactor + appender +curSender; //combining the strings (h,r)
          }
          if(tri_type == "incoming.two.paths"){
            // this extracts all of the past actors that the current sender sent a tie to
            dyadA = curTarget + appender +curactor; //combining the strings (s,h)
            dyadB = curactor + appender +curSender; //combining the strings (h,r)
          }
          if(tri_type == "outgoing.shared.partners"){
            // this extracts all of the past actors that the current sender sent a tie to
            dyadA = curSender + appender +curactor; //combining the strings (s,h)
            dyadB = curTarget + appender +curactor; //combining the strings (h,r)
          }
          ...
          sizegA = past_timesA.size(); 
          for (int g = 0; g < sizegA; g++) { // for all past event times
          ...
          sizegB = past_timesB.size(); 
          for (int g = 0; g < sizegB; g++) { // for all past event times
